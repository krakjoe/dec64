...
global dec64_not;(number: dec64)
;      returns notation: dec64

global dec64_one;()
;      returns one: dec64

global dec64_round;(number: dec64, exponent: dec64)
...
; We use 77/256 to convert log2 to log10.
    ...
    lea     r6,[rel power]
    idiv    qword [r6,r1*8]; divide by the power of ten
    jmp     pack
...
; of digits scaled.
    ...
    lea     r6,[rel power]
    imul    qword [r6,r1*8] ; r2:r0 is the dividend coefficient * 10**r1
...
; coefficient is much larger than the dividend coefficient.
    ...
    lea     r6,[rel power]  ; multiply the dividend by power of ten
    imul    r10,qword [r6,r1*8] ; multiply the dividend by power of ten
...
; Otherwise, return one.
    ...
    jae     dec64_zero
    lea     r6,[rel power]
    mov     r10,qword [r6,r8*8]  ; r10 is 10^-exponent
    cqo                     ; sign extend r0 into r2
